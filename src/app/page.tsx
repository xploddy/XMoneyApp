...
import { Plus, Calendar, Zap, ChevronRight, ChevronLeft, X, LayoutDashboard, Coffee, Home, Car, Heart, Briefcase, HelpCircle, ShoppingCart, ArrowRight, Table } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 py-10 px-4 md:px-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-4 md:pt-0">
        ...
        >
          ...
          <div className="flex flex-col items-start">
            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Período</span>
            <span className="font-black text-sm text-white dark:text-white uppercase tracking-widest">
      ...
      {showMonthModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setShowMonthModal(false)}>
          <div className="w-full max-w-md bg-white dark:bg-[#161B26] rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-black text-slate-900 dark:text-white">Selecionar Período</h3>
              <button onClick={() => setShowMonthModal(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all">
                <X size={20} className="text-slate-400" />
              </button>
            </div>

            {/* Year Selector */}
            <div className="flex items-center justify-between mb-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl">
              <button
              ...
              <span className="text-2xl font-black text-slate-900 dark:text-white">{year}</span>
              <button
                onClick={() => setYear(year + 1)}
                className="p-2 hover:bg-white dark:hover:bg-slate-800 rounded-xl transition-all"
              >
              ...
              {Array.from({ length: 12 }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "py-4 rounded-xl font-black text-sm uppercase tracking-wider transition-all",
                      isSelected
